<template>
    <div
        v-if="isVisible"
        class="overlay">
        <div class="popup cards-popup">
            <h1>{{ $t('image.insertEditcards') }}</h1>

            <div class="cards-popup-images">
                <draggable
                    v-if="!isUploading"
                    tag="ul"
                    group="cards-items"
                    ...
                    ghostClass="is-ghost"
                    handle="img"
                    class="cards-popup-images-list"
                    v-model="images"
                    :data-translation="$t('image.yourcardsIsEmpty')">
                    <li
                        v-for="(image, index) of images"
                        :data-id="index"
                        :key="'images-list-' + index"
                        class="cards-popup-images-list-item">
                        <img
                        ...
                        <div>
                            <input
                                type="text"
                                class="cards-popup-images-list-item-alt"
                                v-model="image.alt"
                                :spellcheck="$store.state.currentSite.config.spellchecking"
                                :placeholder="$t('image.imageAlternativeText')" />
                            <input
                                type="text"
                                class="cards-popup-images-list-item-caption"
                                v-model="image.caption"
                                :spellcheck="$store.state.currentSite.config.spellchecking"
                                :placeholder="$t('image.imageCaption')" />
                        </div>

                        <div class="cards-popup-images-list-operations">
                            <a
                                href="#up"
            ...
            <div class="cards-popup-config">
                <label>
                    {{ $t('image.layout') }}:
                    <select
                        v-model="columns"
                        class="cards-popup-config-cols">
                ...
                <label>
                    ...
                    <select
                        ...
                        <option value="cards-wrapper--wide">{{ $t('image.wide') }}</option>
                        <option value="cards-wrapper--full">{{ $t('image.fullWidth') }}</option>
                    </select>
                </label>
...
export default {
    ...
    mounted () {
        this.$bus.$on('update-cards-popup', async (config) => {
            this.postID = config.postID;
            this.cardsElement = config.cardsElement;
            this.isVisible = true;
    ...
    methods: {
        ...
        parseInputElement () {
            let cardsHandler = $(this.cardsElement);
            let images = $(cardsHandler).find('.cards__item');
            this.columns = cardsHandler.attr('data-columns') || 3;
            this.layout = '';

            if (cardsHandler.hasClass('cards-wrapper--wide')) {
                this.layout = 'cards-wrapper--wide';
            } else if (cardsHandler.hasClass('cards-wrapper--full')) {
                this.layout = 'cards-wrapper--full';
            }
        ...
        save () {
            ...
            this.$bus.$emit('cards-popup-updated', this.generateOutput());
        },

        cancel () {
            this.isVisible = false;
            this.$bus.$emit('cards-popup-updated', false);
        },
        ...
        generateOutput() {
            ...
            for(let i = 0; i < this.images.length; i++) {
                ...
                let item = `<figure class="cards__item">${link}${description}</figure>`;
                output += item;
            }
            ...
            $(this.cardsElement).attr('data-columns', this.columns);
            $(this.cardsElement).removeClass('cards-wrapper--wide').removeClass('cards-wrapper--full');

            if (this.layout !== '') {
                $(this.cardsElement).addClass(this.layout);
            }

            return {
                cards: this.cardsElement,
                html: output
            };
        }
    },
    beforeDestroy () {
        this.$bus.$off('update-cards-popup');
    }
}
</script>
    ...
    &-images-list {
        ...
        &-item {
            ...
            &:first-child {
                ...
                .cards-popup-images-list-operations {
                    & > a[href="#up"] {
                        display: none;
                    }
                }
